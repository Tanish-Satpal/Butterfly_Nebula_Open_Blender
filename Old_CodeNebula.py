...
def scene_bootstrap():
    ...
    n_bg.inputs["Color"].default_value = (0.01, 0.012, 0.018, 1.0)
...
def generate_butterfly_fields(src_obj, num_points=5000):
    ...
    for i in range(num_points):
        ...
        side_len = lobe_length * (0.94 if side < 0.0 else 1.06)
        side_rad = lobe_radius * (1.08 if side < 0.0 else 0.96)
        side_bow = 0.72 if side < 0.0 else 0.56
        ...
        shell_thickness = 0.07 + 0.10 * random.random()
        ...
        n = mnoise.fractal(Vector((x * 0.52, y * 1.6, z * 1.6)), 3.4, 2.0, 4)
        y *= 1.0 + 0.30 * n
        z *= 1.0 + 0.14 * n
        ...
        haze_factor = max(0.0, 1.0 - dist / (lobe_length * 1.08))
        haze_factor = (haze_factor ** 2.4) * (1.0 - 0.55 * axis_hollow)

        radius_vals.append(0.018 + 0.015 * (1.0 - t) + 0.010 * random.random())
        emit_vals.append(0.10 + 0.34 * (1.0 - t) + 0.62 * local_shell)
        dust_vals.append(max(0.0, 0.75 * waist_factor + 0.05 * random.random()))
        haze_vals.append(0.25 * haze_factor + 0.85 * local_shell + 0.25 * local_fil)
    ...
    halo_count = max(int(num_points * 0.10), 320)
    ...
    for j in range(halo_count):
        ...
        spread = lobe_radius * (0.28 + 0.55 * random.random())
        ...
        x_off = math.copysign((0.08 + 0.22 * random.random()) * lobe_length, x0)

        hx = x0 + x_off + 0.15 * mnoise.fractal(Vector((x0, y0, z0)), 2.0, 2.0, 2)
        hy = y0 + y_off
        hz = z0 + z_off

        verts.append((hx, hy, hz))

        radius_vals.append(radius_vals[idx] * 0.75)
        emit_vals.append(emit_vals[idx] * 0.03)
        dust_vals.append(dust_vals[idx] * 0.12)
        haze_vals.append(min(1.0, 0.55 + haze_vals[idx] * 1.35))
...
def write_vdb_files(src_obj, ion_path, dust_path, haze_path, voxel_size=0.05):
    ...
    for i, v in enumerate(verts):
        ...
        vr = max(1, int(math.ceil((radius * 1.25) / voxel_size)))

        # concentrate ion emission into shell/filament walls
        ion_base = emit_w * (0.35 + 0.85 * shell) * (0.45 + 0.55 * filament)

        # keep dust mostly near the waist so it does not muddy the full lobes
        dust_base = dust_w * (0.85 - 0.35 * shell)

        # haze should support outer veils and shell edges, not fill everything evenly
        haze_base = 0.55 * haze_w + 0.90 * shell + 0.22 * filament
...
def build_materials(ion_obj, dust_obj, haze_obj):
    ...
    ramp.elements[0].color = (1.00, 0.98, 0.99, 1.0)
    ramp.elements[1].position = 1.0
    ramp.elements[1].color = (0.72, 0.84, 1.00, 1.0)

    if "Color" in n_principled.inputs:
        n_principled.inputs["Color"].default_value = (0.7, 0.8, 1.0, 1.0)
    if "Density" in n_principled.inputs:
        n_principled.inputs["Density"].default_value = 0.28
    if "Anisotropy" in n_principled.inputs:
        n_principled.inputs["Anisotropy"].default_value = 0.1
    if "Emission Strength" in n_principled.inputs:
        n_principled.inputs["Emission Strength"].default_value = 4.2
    ...
    n3_blackbody = nt3.nodes.new("ShaderNodeBlackbody")
    ...
    n3_noise.inputs["Scale"].default_value = 18.0
    n3_noise.inputs["Detail"].default_value = 8.0
    n3_noise.inputs["Roughness"].default_value = 0.62

    # Noise remap
    n3_noise_map.inputs["From Min"].default_value = 0.34
    n3_noise_map.inputs["From Max"].default_value = 0.88
    n3_noise_map.inputs["To Min"].default_value = 0.0
    n3_noise_map.inputs["To Max"].default_value = 1.55
    n3_noise_map.clamp = True

    # Emission math: emit_mult = noise * gain, emit_final = density * emit_mult
    emit_mult.operation = 'MULTIPLY'
    emit_mult.inputs[0].default_value = 2.2  # gain
    ...
    n3_map.inputs["To Max"].default_value = 0.72
    n3_map.clamp = True

    # Base haze colors
    # Temperature remap: normalized temp 0..1 -> Kelvin
    n3_temp_map.inputs["From Min"].default_value = 0.0
    n3_temp_map.inputs["From Max"].default_value = 1.0
    n3_temp_map.inputs["To Min"].default_value = 3400.0
    n3_temp_map.inputs["To Max"].default_value = 14000.0
    n3_temp_map.clamp = True

    if "Color" in n3_principled.inputs:
        n3_principled.inputs["Color"].default_value = (0.34, 0.46, 0.62, 1.0)
    if "Density" in n3_principled.inputs:
        n3_principled.inputs["Density"].default_value = 0.18
    if "Anisotropy" in n3_principled.inputs:
        n3_principled.inputs["Anisotropy"].default_value = 0.2
    ...
    # Temperature from VDB -> Kelvin -> Blackbody
    nt3.links.new(n3_info.outputs["Temperature"], n3_temp_map.inputs["Value"])
    nt3.links.new(n3_temp_map.outputs["Result"], n3_blackbody.inputs["Temperature"])

    # Density into Principled Volume
    if "Density" in n3_principled.inputs:
        nt3.links.new(n3_map.outputs["Result"], n3_principled.inputs["Density"])

    # Blackbody color into emission color
    if "Emission Color" in n3_principled.inputs:
        nt3.links.new(n3_blackbody.outputs["Color"], n3_principled.inputs["Emission Color"])
    ...
    # Assign materials
    if ion_obj:
        ion_obj.data.materials.clear()
        ion_obj.data.materials.append(mat_ion)
    if dust_obj:
...
def create_star_field():
    ...
    for i in range(star_count):
        ...
        bpy.ops.mesh.primitive_uv_sphere_add(
            radius=0.0025 + random.random() * 0.0035,
            location=(
        ...
        t = random.random()
        if t < 0.10:
            # cool red/orange (M/K-type)
            col = (1.0, 0.73, 0.52, 1.0)
            strength = 4.0 + random.random() * 10.0
        elif t < 0.25:
            # warm orange-yellow
            col = (1.0, 0.82, 0.62, 1.0)
            strength = 5.0 + random.random() * 12.0
        elif t < 0.65:
            # near-solar white
            col = (1.0, 0.96, 0.90, 1.0)
            strength = 3.5 + random.random() * 9.0
        elif t < 0.90:
            # cool white / slightly blue
            col = (0.86, 0.93, 1.0, 1.0)
            strength = 4.0 + random.random() * 11.0
        else:
            # hot blue
            col = (0.72, 0.84, 1.0, 1.0)
            strength = 6.0 + random.random() * 16.0
...
def create_central_star():
            ...
            bpy.ops.mesh.primitive_uv_sphere_add(radius=0.012, location=(0.0, 0.0, 0.02))
            star = bpy.context.active_object
            ...
            n_em.inputs["Color"].default_value = (1.0, 0.94, 0.86, 1.0)
            n_em.inputs["Strength"].default_value = 90.0
...
def setup_compositor():
    ...
    n_glare.mix = 0.5
    n_glare.threshold = 0.9
    n_glare.size = 7

    if hasattr(n_cb, "correction_method"):
        n_cb.correction_method = 'LIFT_GAMMA_GAIN'
    n_cb.lift = (0.98, 0.99, 1.02)
    n_cb.gamma = (0.98, 1.0, 1.04)
    n_cb.gain = (1.04, 1.02, 0.99)

    curve = n_curves.mapping.curves[3]
    curve.points.new(0.18, 0.22)
    curve.points.new(0.82, 0.78)
